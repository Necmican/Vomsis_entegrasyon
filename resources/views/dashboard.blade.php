...
<body class="bg-light pb-5">
        ...
        <div class="container-fluid px-4">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    <li class="nav-item"><a class="nav-link active" href="{{ url('/dashboard') }}">📊 Hesap
                            Hareketleri</a></li>

                    @if(auth()->user()->can_view_pos || auth()->user()->role === 'admin')
                        <li class="nav-item"><a class="nav-link text-white-50" href="{{ route('payment.list') }}">💳 Sanal
                                POS İşlemleri</a></li>
                    @endif

                    @if(auth()->user()->role === 'admin' || auth()->user()->can_view_physical_pos)
                        <li class="nav-item"><a class="nav-link text-white-50" href="{{ route('physical_pos.index') }}">📠
                                Fiziksel POS</a></li>
                    @endif

                    <li class="nav-item"><a class="nav-link text-white-50" href="{{ route('analytics.index') }}">📈
                            Analizler</a></li>

                    @if(auth()->user()->role === 'admin')
                        <li class="nav-item"><a class="nav-link text-white-50" href="{{ route('auto-tag.index') }}">
                                Oto-Etiket</a></li>
                    @endif
                </ul>
                ...
                <div class="d-flex align-items-center gap-2">
                    @if(auth()->user()->role === 'admin')
                        ...
                        <button type="button" class="btn btn-outline-warning btn-sm fw-bold" data-bs-toggle="modal"
                            data-bs-target="#bankSettingsModal">
                            ⚙️ Kasa Yönetimi
                        </button>
                    @endif
                    ...
                    <div class="dropdown ms-2">
                        <button class="btn btn-secondary btn-sm dropdown-toggle fw-bold" type="button"
    ...
    <div class="container-fluid px-4 mt-4">
        ...
        <!-- KUYRUK (QUEUE) DURUM BİLDİRİM BÖLÜMÜ -->
        <div id="exportStatusContainer"
            class="d-none alert shadow-sm p-3 mb-4 d-flex justify-content-between align-items-center border-0"
            style="border-left: 4px solid #0d6efd !important; background: #f8f9fa;">
            <div class="d-flex align-items-center gap-3">
                <div id="exportStatusSpinner" class="spinner-border text-primary spinner-border-sm" role="status"></div>
                <div id="exportStatusIcon" class="fs-4 d-none">✅</div>
                <div>
            ...
            <div>
                <a id="exportDownloadBtn" href="#" class="btn btn-sm btn-success fw-bold d-none shadow-sm px-4">📥
                    İNDİR</a>
            </div>
        </div>
        ...
        <div class="row">
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="list-group list-group-flush accordion" id="bankMenuAccordion">
                        <a href="{{ url('/dashboard') }}"
                            class="list-group-item list-group-item-action py-3 bank-item {{ (!request('bank_id') && !request('account_id')) ? 'active' : '' }}">
                        ...
                        @foreach($banks as $bank)
                            <div class="accordion-item border-0">
                                <h2 class="accordion-header" id="headingBank{{ $bank->id }}">
                                    <button
                                        class="accordion-button list-group-item list-group-item-action py-3 bank-item {{ (request('bank_id') == $bank->id || (isset($activeBank) && $activeBank->id == $bank->id) || request('account_id')) ? '' : 'collapsed' }}"
                                        type="button" data-bs-toggle="collapse"
                                        data-bs-target="#collapseBank{{ $bank->id }}" style="box-shadow: none;">
                                        <div class="d-flex w-100 justify-content-between align-items-center pe-2">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span style="font-size: 0.9rem;"
                                                    class="{{ (request('bank_id') == $bank->id || (isset($activeBank) && $activeBank->id == $bank->id)) ? 'fw-bold text-primary' : '' }}">{{ $bank->bank_name }}</span>
                                            </div>
                                            <span
                                                class="badge bg-secondary rounded-pill">{{ $bank->bankAccounts->count() }}</span>
                                        </div>
                                    </button>
                                </h2>
                                ...
                                <div id="collapseBank{{ $bank->id }}"
                                    class="accordion-collapse collapse {{ $isMenuOpen ? 'show' : '' }}"
                                    data-bs-parent="#bankMenuAccordion">
                                    <div class="accordion-body p-0">
                                        <div class="list-group list-group-flush bg-light border-bottom">
                                            <a href="{{ url('/dashboard?bank_id=' . $bank->id) }}"
                                                class="list-group-item list-group-item-action py-2 ps-4 {{ request('bank_id') == $bank->id && !request('account_id') ? 'active fw-bold' : 'text-primary border-0 bg-transparent' }}"
                                                style="font-size: 0.85rem;">
                                                ➡️ Tüm Banka İşlemleri
                                            </a>
                                            @foreach($bank->bankAccounts as $acc)
                                                @if($acc->is_visible)
                                                    <a href="{{ url('/dashboard?account_id=' . $acc->id) }}"
                                                        class="list-group-item list-group-item-action py-2 ps-4 border-0 {{ request('account_id') == $acc->id ? 'active fw-bold' : 'bg-transparent' }}"
                                                        style="font-size: 0.82rem;">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <span
                                                                class="text-truncate {{ request('account_id') == $acc->id ? 'text-white' : 'text-muted' }}"
                                                                style="max-width: 110px;"
                                                                title="{{ $acc->account_name }}">{{ Str::limit($acc->account_name, 15) }}</span>
                                                            <strong
                                                                class="{{ request('account_id') == $acc->id ? 'text-white' : 'text-dark' }}">{{ number_format($acc->balance, 2, ',', '.') }}
                                                                <small>{{ $acc->currency }}</small></strong>
                                                        </div>
                                                    </a>
                                                @endif
                                            @endforeach
            ...
            <div class="col-md-9 col-lg-10">
                            ...
                            <div class="row">
                                <div class="col-md-5 border-end pe-4">
                                    ...
                                    @foreach($activeBankSummary as $curr => $data)
                                        <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                            ...
                                            <span
                                                class="fw-bold fs-5 text-primary">{{ number_format($data['total'], 2, ',', '.') }}
                                                {{ $curr }}</span>
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                <div class="col-md-7 ps-4">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        @if(request('account_id'))
                                            <a href="{{ url('/dashboard?bank_id=' . $activeBank->id) }}"
                                                class="badge bg-danger text-decoration-none">Filtreyi Kaldır ✕</a>
                                        @endif
                                    </div>

                                    @foreach($activeBankAccounts as $acc)
                                        <a href="{{ request()->fullUrlWithQuery(['account_id' => $acc->id]) }}"
                                            class="account-card-link">
                                            <div
                                                class="card border {{ request('account_id') == $acc->id ? 'border-primary border-2 shadow' : 'border-light shadow-sm bg-light' }} mb-2 rounded-3 account-card">
                                                <div
                                                    class="card-body py-2 px-3 d-flex justify-content-between align-items-center">
                                                    <div class="d-flex align-items-center">
                                                        <span
                                                            class="{{ request('account_id') == $acc->id ? 'text-primary' : 'text-secondary' }} fw-bold me-2">
                                                            {{ $acc->account_name }}
                                                        </span>
                                                    </div>
                                                    <div class="text-end">
                                                        <strong
                                                            class="fs-6 {{ request('account_id') == $acc->id ? 'text-primary' : 'text-dark' }}">
                                ...
                                <div class="card shadow-sm border-0 border-start border-info border-4 h-100">
                                    <div class="card-body py-2">
                                        <h6 class="text-muted text-uppercase mb-2 fw-bold" style="font-size: 0.8rem;">
                                            <span class="badge bg-info text-dark me-1">{{ $summary->currency }}</span> İşlem
                                            Özeti (Bu Liste)
                                        </h6>
                                        <div class="d-flex justify-content-between">
                                            <div><small
                                                    class="text-success d-block fw-bold">+{{ number_format($summary->total_income, 2, ',', '.') }}</small>
                                            </div>
                                            <div><small
                                                    class="text-danger d-block fw-bold">{{ number_format($summary->total_expense, 2, ',', '.') }}</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <form action="{{ url('/dashboard') }}" method="GET" class="row g-2 align-items-center">
                            ...
                            <div class="col-md-3">
                                <input type="text" name="search" class="form-control form-control-sm"
                                    placeholder="Açıklamada ara..." value="{{ request('search') }}">
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                <select name="type_code" class="form-select form-select-sm">
                                    ...
                                    @foreach($transactionTypes as $type)
                                        <option value="{{ $type->vomsis_type_id }}" {{ request('type_code') == $type->vomsis_type_id ? 'selected' : '' }}>
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-2 d-flex gap-2">
                                ...
                                @if(request('search') || request('currency') || request('type_code') || request('account_id') || request('tag_id'))
                                    <a href="{{ url('/dashboard') }}{{ request('bank_id') ? '?bank_id=' . request('bank_id') : '' }}"
                        ...
                        <div class="row mt-3 border-top pt-3">
                            <div class="col-12 d-flex justify-content-end gap-2">
                                <button type="button" class="btn btn-outline-danger btn-sm fw-bold"
                                    onclick="startExport('pdf', '{{ request()->getQueryString() ?? '' }}')">
                                    <i class="fas fa-file-pdf"></i> PDF İNDİR
                                </button>

                                <button type="button" class="btn btn-outline-success btn-sm fw-bold"
                                    data-bs-toggle="modal" data-bs-target="#excelExportModal">
                ...
                <div id="bulkActionContainer"
                    class="d-none mb-3 p-3 bg-white shadow-sm rounded border border-primary d-flex justify-content-between align-items-center"
                    style="transition: all 0.3s ease;">
                    <div class="d-flex align-items-center">
                        <span class="fs-5 me-2">⚙️</span>
                        <span id="selectedCountText" class="fw-bold text-primary" style="font-size: 1.1rem;">0 işlem
                            seçildi</span>
                        <button type="button" id="clearSelectionBtn"
                            class="btn btn-sm btn-link text-danger ms-3 fw-bold text-decoration-none">❌ Temizle</button>
                    </div>
                    <div>
                        <button type="button" class="btn btn-outline-danger fw-bold shadow-sm me-2"
                            data-bs-toggle="modal" data-bs-target="#bulkRemoveTagModal">
                            🗑️ Etiket Çıkar
                        </button>
                        ...
                        <table class="table table-hover table-bordered mb-0 align-middle">
                            ...
                            <tbody>
                                @forelse($transactions as $islem)
                                    <tr class="table-row-clickable" data-modal-id="islemModal{{ $islem->id }}"
                                        data-id="{{ $islem->id }}">

                                        <td class="text-center action-checkbox-cell">
                                            <input type="checkbox" value="{{ $islem->id }}"
                                                class="form-check-input transaction-checkbox shadow-sm"
                                                style="transform: scale(1.2); cursor: pointer;"
                                                onclick="event.stopPropagation();">
                                        </td>
                                        ...
                                        <td>
                                            <strong>{{ ucfirst($islem->bankAccount->bank->bank_name ?? 'Bilinmeyen') }}</strong>
                                            <br>
                                            <small class="text-muted">{{ $islem->bankAccount->account_name }}
                                                ({{ $islem->bankAccount->currency }})</small>
                                        </td>
                                        <td>
                                            <span
                                                class="badge bg-secondary">{{ $islem->transactionType->name ?? 'Diğer' }}</span>
                                            <div class="mt-1 d-flex flex-wrap gap-1">
                                                @foreach($islem->tags as $tag)
                                                    <span class="badge"
                                                        style="background-color: {{ $tag->color }}; font-size: 0.7rem;">{{ $tag->name }}</span>
                                                @endforeach
                                            </div>
                                        </td>
                                        <td class="text-truncate" style="max-width: 250px;">{{ $islem->description }}</td>
                                        <td class="text-end pe-4">
                                            <span class="fw-bold {{ $islem->amount > 0 ? 'text-success' : 'text-danger' }}"
                                                style="font-size: 1.05rem;">
                                                {{ $islem->amount > 0 ? '+' : '' }}{{ number_format($islem->amount, 2, ',', '.') }}
                                            </span><br>
                                            <small class="text-muted" style="font-size: 0.8rem;">Bakiye:
                                                {{ number_format($islem->balance, 2, ',', '.') }}
                                                {{ $islem->bankAccount->currency }}</small>
                                        </td>
                                    </tr>

                                    <div class="modal fade" id="islemModal{{ $islem->id }}" tabindex="-1"
                                        aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow-lg">
                                                ...
                                                <div class="modal-body bg-light">
                                                    <div
                                                        class="small text-muted mb-3 pb-2 border-bottom fw-bold text-uppercase">
                                                        🏦 Banka #{{ $islem->bankAccount->bank->id }} > 💳 Hesap
                                                        #{{ $islem->bankAccount->id }} > 📄 İşlem #{{ $islem->id }}
                                                    </div>
                                                    <ul class="list-group list-group-flush rounded shadow-sm border">
                                                        <li
                                                            ...
                                                            <strong
                                                                class="text-end">{{ $islem->bankAccount->bank->bank_name }}</strong>
                                                        </li>
                                                        <li
                                                            class="list-group-item d-flex justify-content-between align-items-center py-3">
                                                            <span class="text-muted">Hesap Adı:</span>
                                                            <strong
                                                                class="text-end">{{ $islem->bankAccount->account_name }}</strong>
                                                        </li>
                                                        <li
                                                            class="list-group-item d-flex justify-content-between align-items-center py-3">
                                                        ...
                                                        <li
                                                            ...
                                                            @if($islem->amount > 0)
                                                                <span class="badge bg-success fs-6 px-3 py-2">GELEN
                                                                    (ALACAKLI)</span>
                                                            @else
                                                                <span class="badge bg-danger fs-6 px-3 py-2">GİDEN
                                                                    (BORÇLU)</span>
                                                            @endif
                                                        </li>
                                                        ...
                                                        <li
                                                            class="list-group-item d-flex justify-content-between align-items-center py-3 bg-white">
                                                            <span class="text-muted fs-5">İşlem Tutarı:</span>
                                                            <strong
                                                                class="fs-4 {{ $islem->amount > 0 ? 'text-success' : 'text-danger' }}">
                                                                {{ $islem->amount > 0 ? '+' : '' }}{{ number_format($islem->amount, 2, ',', '.') }}
                                                                {{ $islem->bankAccount->currency }}
                                                            </strong>
                                                        </li>

                                                        <li class="list-group-item py-3 bg-white">
                                                            <span class="text-muted d-block mb-2 fw-bold">🏷️ İşlem
                                                                Etiketleri:</span>

                                                            <div class="d-flex flex-wrap gap-2 mb-3">
                                                                @forelse($islem->tags as $tag)
                                                                    <span class="badge d-flex align-items-center py-2 px-2"
                                                                        style="background-color: {{ $tag->color }}; font-size: 0.85rem;">
                                                                        {{ $tag->name }}
                                                                        <form
                                                                            action="{{ url('/islem/' . $islem->id . '/etiket-cikar/' . $tag->id) }}"
                                                                            method="POST" class="ms-2 m-0 p-0 d-inline">
                                                                            @csrf
                                                                            <button type="submit"
                                                                                class="btn-close btn-close-white"
                                                                                style="font-size: 0.5rem;"
                                                                                title="Bu etiketi kaldır"></button>
                                                                        </form>
                                                                    </span>
                                                                @empty
                                                                    <span class="text-muted small">Bu işleme henüz etiket
                                                                        atanmamış.</span>
                                                                @endforelse
                                                            </div>

                                                            <form
                                                                action="{{ url('/islem/' . $islem->id . '/etiket-ekle') }}"
                                                                ...
                                                                <button type="submit"
                                                                    class="btn btn-sm btn-primary text-nowrap fw-bold shadow-sm">+
                                                                    Ekle</button>
                                                            </form>
                                                        </li>
                                                    </ul>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center py-5 text-muted">İşlem bulunamadı.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div id="ai-chat-window">
            ...
            <div class="ai-body" id="ai-messages-container">
                <div class="chat-msg msg-ai">
                    Merhaba {{ auth()->user()->name }}! Ben Vomsis Yapay Zeka Asistanı. Hesap hareketleri veya finansal
                    özetler hakkında bana her şeyi sorabilirsin.
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width: 500px;">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
                <div class="modal-header border-bottom-0 pb-0 mt-2">
                    <h5 class="modal-title w-100 text-center fw-bold" style="font-size: 1.1rem; color: #333;">Şablon
                        Seçin</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body px-5 pb-5">

                    <form id="excelExportForm" onsubmit="event.preventDefault(); submitExcelExport();">
                        @csrf
                        <input type="hidden" name="bank_id" value="{{ request('bank_id') }}">
                        <input type="hidden" name="account_id" value="{{ request('account_id') }}">
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="separate_banks" value="1"
                                id="sepBanks" style="transform: scale(1.2); margin-right: 8px;">
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="separate_accounts" value="1"
                                id="sepAccounts" style="transform: scale(1.2); margin-right: 8px;">
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" value="1" id="incSafes" disabled
                        ...
                        <div class="text-center mt-4">
                            <small class="text-muted d-block mb-3" style="font-size: 0.85rem;">Excel sütunlarını
                                özelleştirmek istiyorsanız şablon oluşturun.</small>
                            <button type="submit" class="btn text-white fw-bold px-5 py-2 shadow-sm"
                                style="background-color: #3fb0ff; border-radius: 8px; font-size: 0.95rem;">
                                EXCEL ÇIKART
                            </button>
                        </div>
                    </form>
                ...
                <form action="{{ route('bank.settings.update') }}" method="POST" class="modal-content border-0 shadow-lg">
                    ...
                    <div class="modal-body bg-light p-4">
                        <div class="alert alert-warning border-0 shadow-sm mb-4">
                            <i class="fas fa-exclamation-triangle me-2"></i> Kapattığınız banka veya hesaplar listeden ve
                            hesaplamalardan (Gelir/Gider/Kasa) tamamen düşer.
                        </div>
                                ...
                                @foreach($allSettingsBanks as $bank)
                                    <div class="accordion-item border-0 mb-2 rounded">
                                        <h2 class="accordion-header" id="heading{{ $bank->id }}">
                                            <div
                                                class="d-flex align-items-center justify-content-between w-100 bg-white border rounded px-3 py-2">
                                                <button
                                                    class="accordion-button collapsed shadow-none bg-transparent p-0 m-0 w-75 fw-bold"
                                                    type="button" data-bs-toggle="collapse"
                                                    data-bs-target="#collapse{{ $bank->id }}">
                                                    🏦 {{ $bank->bank_name }}
                                                </button>
                                                <div
                                                    class="form-check form-switch m-0 d-flex align-items-center w-25 justify-content-end">
                                                    <label class="form-check-label small text-muted me-2"
                                                        style="font-size:0.75rem;">Listede Göster</label>
                                                    <input class="form-check-input" type="checkbox"
                                                        name="banks[{{ $bank->id }}][is_visible]" value="1" {{ $bank->is_visible ? 'checked' : '' }} style="transform: scale(1.3); cursor: pointer;">
                                                </div>
                                            </div>
                                        </h2>
                                        <div id="collapse{{ $bank->id }}" class="accordion-collapse collapse"
                                                            ...
                                                            <tr class="border-bottom">
                                                                ...
                                                                <td class="align-middle text-center">
                                                                    <div class="form-check form-switch d-inline-block m-0">
                                                                        <input class="form-check-input" type="checkbox"
                                                                            name="accounts[{{ $acc->id }}][is_visible]" value="1" {{ $acc->is_visible ? 'checked' : '' }}
                                                                            style="transform: scale(1.2); cursor: pointer;">
                                                                    </div>
                                                                </td>
                                                                <td class="align-middle text-center">
                                                                    <div class="form-check form-switch d-inline-block m-0">
                                                                        <input class="form-check-input bg-success border-success"
                                                                            type="checkbox"
                                                                            name="accounts[{{ $acc->id }}][include_in_totals]" value="1"
                                                                            {{ $acc->include_in_totals ? 'checked' : '' }}
                                                                            style="transform: scale(1.2); cursor: pointer;">
                                                                    </div>
                                                                </td>
                                                            </tr>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="color" name="color" class="form-control form-control-color w-100"
                                    value="#0d6efd" title="Renginizi seçin" required style="height: 45px;">
                            </div>
                        </div>
                ...
                <form action="{{ url('/islem/toplu-etiket-ekle') }}" method="POST" id="bulkTagForm">
                    ...
                    <div class="modal-body bg-light">
                        <div class="alert alert-info py-2 mb-4 shadow-sm border-0">
                            <i class="fas fa-info-circle me-2"></i> <strong id="modalSelectedCount">0</strong> adet
                            işleme aşağıdaki etiketler eklenecektir.
                        </div>

                        <label class="form-label text-muted small fw-bold mb-2">Eklenecek Etiketleri Seçin (Çoklu Seçim
                            Yapabilirsiniz)</label>
                        ...
                        <div class="bg-white p-3 rounded border shadow-sm" style="max-height: 250px; overflow-y: auto;">
                            @if(isset($allTags) && $allTags->count() > 0)
                                @foreach($allTags as $tag)
                                    <div class="form-check mb-2 custom-control custom-checkbox">
                                        <input class="form-check-input" type="checkbox" name="tag_ids[]" value="{{ $tag->id }}"
                                            id="bulkTag_{{ $tag->id }}" style="cursor: pointer; transform: scale(1.1);">
                                        <label class="form-check-label ms-2 w-100" for="bulkTag_{{ $tag->id }}"
                                            style="cursor: pointer;">
                                            <span class="badge py-2 px-3 shadow-sm w-100 text-start"
                                                style="background-color: {{ $tag->color }}; font-size: 0.85rem;">
                                                {{ $tag->name }}
                                            </span>
                                        </label>
                                ...
                                @endforeach
                            @else
                                <div class="text-muted small text-center py-3">Sistemde yetkiniz olan hiçbir etiket
                                    bulunmuyor.</div>
                            @endif
                        </div>
                ...
                <form action="{{ url('/islem/toplu-etiket-cikar') }}" method="POST" id="bulkRemoveTagForm">
                    ...
                    <div class="modal-body bg-light">
                        <div class="alert alert-warning py-2 mb-4 shadow-sm border-0">
                            <i class="fas fa-exclamation-triangle me-2"></i> <strong
                                id="modalRemoveSelectedCount">0</strong> adet işlemden aşağıdaki etiketler silinecektir.
                        </div>
                        ...
                        <div class="bg-white p-3 rounded border shadow-sm" style="max-height: 250px; overflow-y: auto;">
                            @if(isset($allTags) && $allTags->count() > 0)
                                @foreach($allTags as $tag)
                                    <div class="form-check mb-2 custom-control custom-checkbox">
                                        <input class="form-check-input" type="checkbox" name="tag_ids[]" value="{{ $tag->id }}"
                                            id="bulkRemoveTag_{{ $tag->id }}" style="cursor: pointer; transform: scale(1.1);">
                                        <label class="form-check-label ms-2 w-100" for="bulkRemoveTag_{{ $tag->id }}"
                                            style="cursor: pointer;">
                                            <span class="badge py-2 px-3 shadow-sm w-100 text-start"
                                                style="background-color: {{ $tag->color }}; font-size: 0.85rem;">
                                                {{ $tag->name }}
                    ...
                    <div class="modal-footer border-0 bg-light pt-0">
                        <button type="button" class="btn btn-secondary fw-bold" data-bs-dismiss="modal">İptal</button>
                        <button type="submit" class="btn btn-danger fw-bold shadow-sm px-4">Seçili Etiketleri
                            Çıkar</button>
                    </div>
                </form>
            </div>
    ...
    <ul id="customContextMenu" class="dropdown-menu shadow"
        style="display:none; position:absolute; z-index:9999; cursor: pointer;">
        <li>
            ...
            <div class="modal-content border-0 shadow-lg">
                <form id="emailForm" method="POST">
                    ...
                    <div class="modal-body bg-light">
                        ...
                        <small class="text-muted" style="font-size: 0.75rem;">Dekont PDF olarak eke
                            eklenecektir.</small>
                    </div>
                    <div class="modal-footer border-0 bg-light pt-0">
    ...
    <script>
        // --- ARKA PLAN (QUEUE) DIŞA AKTARIM (EXPORT) MANTIĞI ---
        let exportPollInterval = null;

        function startExport(type, queryString) {
            // PDF için
            if (type === 'pdf') {
                fetch(`/export/pdf?${queryString}`, {
                    method: 'GET',
        ...
        function submitExcelExport() {
            const formRow = document.getElementById('excelExportForm');
            ...
            fetch(`/export/excel`, {
                ...
                .then(data => {
                    if (data.success) {
                        // Modalı kapat
                        const modalEl = document.getElementById('excelExportModal');
                        const modalObj = bootstrap.Modal.getInstance(modalEl);
                        if (modalObj) modalObj.hide();

                        // UI Göster ve Takibe başla
                        showExportStatusUI('pending');
                        startPollingExportStatus();
                    }
                })
        ...
        function showExportStatusUI(status, fileUrl = null, percentage = 0, autoDownload = true) {
            ...
            if (status === 'pending' || status === 'processing') {
                ...
                icon.classList.remove('d-none');
                icon.innerText = '❌';
                title.innerText = 'İşlem Başarısız!';
        ...
        function startPollingExportStatus() {
            if (exportPollInterval) return; // Zaten çalışıyorsa başlatma
            checkExportStatus(); // Hemen ilk kontrolü yap
            exportPollInterval = setInterval(checkExportStatus, 3000); // 3 saniyede bir sor
        }
        ...
        function checkExportStatus() {
            fetch('/export/status', {
                ...
                .then(data => {
                    ...
                    // Sadece en son işleme bakalım (şimdilik)
        ...
        // Sayfa yüklendiğinde arka planda kalan iş var mı kontrol et
        document.addEventListener('DOMContentLoaded', () => {
            fetch('/export/status', { headers: { 'Accept': 'application/json' } })
                .then(res => res.json())
                .then(data => {
