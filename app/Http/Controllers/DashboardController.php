...
{
    /**
     * Hesap ID listesi verildiğinde, her hesabın KRONOLOJİK olarak en son
     * işlemindeki bakiyesini döndürür.
     */
    private function getLatestBalances(array $accountIds): array
    {
        ...
        foreach ($accountIds as $accId) {
            $bal = Transaction::where('bank_account_id', $accId)
    ...
    public function index(Request $request)
    {
        ...
        // ========================================================================
        $banksQuery = Bank::with([
            'bankAccounts' => function ($q) {
                $q->where('is_visible', true)
                  ->whereHas('transactions'); // Global Scope otomatik devreye girer
            }
        ])->where('is_visible', true)
          ->whereHas('bankAccounts', function ($q) {
              $q->where('is_visible', true)
                ->whereHas('transactions'); // İçinde işlem olmayan bankaları da gizle
          })
          ->orderBy('bank_name');
        ...
        $validAccountQuery = BankAccount::where('include_in_totals', true)
            ->where('is_visible', true)
        ...
        $summaryQuery = Transaction::whereIn('bank_account_id', $validAccountIds);
        ...
        if ($request->filled('type_code'))
            $summaryQuery->where('transaction_type_code', $request->type_code);
        if ($request->filled('account_id'))
            $summaryQuery->where('bank_account_id', $request->account_id);
        if ($request->filled('tag_id'))
            $summaryQuery->whereHas('tags', fn($q) => $q->where('tags.id', $request->tag_id));

        // Önbellek Anahtarı Oluştur (Seçili filtreler değiştikçe yeni bir tablo hesaplar)
        $cacheKey = 'dashboard_summaries_' . auth()->id() . '_' . md5(json_encode($request->all()) . json_encode($validAccountIds));

        $filteredSummariesArray = \Illuminate\Support\Facades\Cache::remember($cacheKey, now()->addMinutes(10), function () use ($summaryQuery, $validAccountCurrencies) {
            ...
            return collect($totalsByCurrency)->map(fn($val, $key) => (object) [
                'currency' => $key,
        ...
        $filteredSummaries = collect($filteredSummariesArray);
    ...
    public function updateBankSettings(Request $request)
    {
        try {
            ...
            $visibleBankIds = array_keys(array_filter($banks, function ($b) {
                return isset($b['is_visible']); }));

            $visibleAccountIds = array_keys(array_filter($accounts, function ($a) {
                return isset($a['is_visible']); }));
            $includeTotalIds = array_keys(array_filter($accounts, function ($a) {
                return isset($a['include_in_totals']); }));

            // 4. Sadece açık olan şalterlerin ID'lerini veritabanında "Açık (True)" yapıyoruz
            if (!empty($visibleBankIds)) {
                Bank::whereIn('id', $visibleBankIds)->update(['is_visible' => true]);
            }
            if (!empty($visibleAccountIds)) {
                BankAccount::whereIn('id', $visibleAccountIds)->update(['is_visible' => true]);
            }
            if (!empty($includeTotalIds)) {
                BankAccount::whereIn('id', $includeTotalIds)->update(['include_in_totals' => true]);
            }
    ...
    {
        set_time_limit(300); // PDF oluşturma uzun sürebilir, sınırı artır
        $transaction = Transaction::with(['bankAccount.bank', 'transactionType', 'tags'])->findOrFail($id);
        $this->checkTransactionAccess($transaction);
    ...
    {
        set_time_limit(300); // PDF oluşturma ve mail atma uzun sürebilir, sınırı artır
